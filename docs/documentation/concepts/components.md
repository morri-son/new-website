...
In OCM, *components* group a set of semantically related *component versions*. Each component version is uniquely and globally identified by a [*component identity*](/documentation/concepts/coordinates) and can reference other components. A component version can also contain *artifacts* and a formal description of how to access them. These artifacts come in two categories: *resources*, which describe the payload (e.g., OCI images), and *sources*, which describe the input for creating resources (e.g., source code).
